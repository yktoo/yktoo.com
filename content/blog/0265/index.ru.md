...
Однажды наступил день (а вернее, ночь), когда пришлось заменить штатный детектор дыма (или, по-научному, [пожарный извещатель][wiki:ru:Пожарный_извещатель]) у нас дома, установленный ещё при его строительстве.
...
До того не особенно известная компания [Nest Labs][wiki:en:Nest_Labs] (основанная, кстати, двумя выходцами из Apple) обратила на себя внимание в прошлом году — после того, как была приобретена Гуглом за фантастическую сумму в 3,2 млрд долларов. Первым и наиболее известным продуктом //Nest Labs// был «умный» термостат (регулятор температуры в помещении) [Nest Thermostat](https://nest.com/thermostat/meet-nest-thermostat/) необычного дизайна.

Потом последовал детектор дыма //Nest Protect// (первая версия), тоже претендующий на «умность» и напичканный довольно необычными для такого класса устройств функциями. После некоторых сложностей на старте, связанных с безопасностью устройства (например, они запретили функцию //Wave Silence//, позволявшую отключить сирену при ложной тревоге, просто помахав перед устройством рукой, поскольку надёжность её реализации вызывала сомнения), оно стало довольно популярным в Европе и США.

## Nest Protect 2 {#nest-protect-2}

А в июне 2015 года компания выпустила вторую версию [Nest Protect 2](https://nest.com/blog/2015/06/17/meet-the-new-nest-protect/), отличающуюся более тонким корпусом, неким «датчиком раздельного спектра» (//split spectrum sensor//) вместо обычного, функцией самотестирования, датчиком температуры и в целом более умным программным обеспечением.
...
Немного отпугивала цена: в Нидерландах одно такое чудо-устройство стоит **€119**, что для детектора дыма (обычная цена 5-10 евро) немного того… С другой стороны, он также обнаруживает [угарный газ][wiki:ru:Монооксид_углерода], который намного более опасен, поскольку не обладает запахом, а цены на такие детекторы уже заметно выше (от 25 евро за самые примитивные и до 50-60 за продвинутые, с Bluetooth и т.п.).

Но, ознакомившись со спецификацией //Nest Protect 2//, я решил, что своих денег он, пожалуй, стоит. Итак,

## Спецификации {#specs}

//Nest Protect 2// существует в двух вариантах:
...
Поскольку у меня в потолке имеется специальное гнездо для подключения детектора к сети, я приобрёл версию //Wired//.

|**Датчики**            |
|-----------------------|------------------------------------------------------------------------------------------
|- Датчик дыма          | Двойной (//split spectrum sensor//), 450 и 880 нм
...
//Nest Protect 2// использует отдельный датчик влажности, чтобы исключить ложные срабатывания детектора дыма от пара — например, из ванной ([исследование в PDF](https://s3.amazonaws.com/support-assets.nest.com/images/tpzimages/tpz-steam-check-white-paper.pdf)). Утверждается, что используемый алгоритм снижает вероятность ложной тревоги на 50%.

### Датчик температуры

Устройство способно обнаруживать «аномальные температуры» и соответствующим образом информировать //Nest Thermostat// (при его наличии). Например, если дома слишком жарко или холодно настолько, что вода в трубах может замёрзнуть.

### Датчик движения

//Nest Protect 2// может работать ночником (//Pathlight//), освещая дорогу своим светодиодных кольцом, когда кто-нибудь проходит мимо. Также может сообщать термостату, что дома есть люди.

### Датчик освещённости

Нужен для работы вышеупомянутой функции ночника. Кроме того, когда выключаешь свет перед сном, прибор показывает, что всё работает нормально, зажигая кольцо зелёного цвета (//Nightly Promise//).
...
Работает устройство на процессоре *Freescale Kinetis K60*, для интерконнекта используется протокол [ZigBee][wiki:ru:ZigBee]. С учётом всего вышеперечисленного, цена уже далеко не кажется высокой.
...
![](img:4.bp.blogspot.com/-lN_q6pfT7xY/VlrPOcGkqGI/AAAAAAAAk2c/ISvs0rB2e1o/s1600/dsc01086.picasaweb.jpg:a "Nest Protect 2 в упаковке.")

Значок кабеля в левом нижнем углу коробки, похожий на логотип этого сайта, означает что это версия //Wired//, которая требует подключения к сети.

Ящичек заклеен плёнкой, так что незаметно открыть его не получится.
![](img:3.bp.blogspot.com/-iBqZ79TDlr0/VlrPOdife5I/AAAAAAAAk2c/jbtS6_zZngs/s1600/dsc01087.picasaweb.jpg:a "Коробка опечатана.")
...
![](img:2.bp.blogspot.com/-9_QnXCiCCKI/VlrPOVGhhLI/AAAAAAAAk2c/DxQlYbaHRyI/s1600/dsc01088.picasaweb.jpg:a "Конверт с руководствами.")

Под конвертом лежит наш герой, прикрытый пластиковой крышкой.
![](img:3.bp.blogspot.com/-G88VFohIu7Q/VlrPOfHij-I/AAAAAAAAk2c/-23POZ_4rNk/s1600/dsc01089.picasaweb.jpg:a "Nest Protect 2 в коробке.")
...
![](img:3.bp.blogspot.com/-ImuPhW_9UB4/VlrPObg7G-I/AAAAAAAAk2c/9zKayOYkobw/s1600/dsc01091.picasaweb.jpg:a "Технологический фетишизм.")

Вот всё содержимое упаковки:
![](img:2.bp.blogspot.com/-VBwVfgZfyzc/VlrPOeG9xEI/AAAAAAAAk2c/d54tBPqsYsw/s1600/dsc01092.picasaweb.jpg:a "Комплект поставки.")

Рассмотрим главного героя со стороны дна. Сверху технологический разъём //Mini USB//, по обе стороны от него тонна информации о видах предупреждений и индикации на французском и голландском, в центре разъём питания от сети, внизу откидывающийся отсек для батарей резервного питания, на котором расположен индивидуальный QR-код детектора.
![](img:3.bp.blogspot.com/-WVSGSFKMbhs/VlrPOQJrZtI/AAAAAAAAk2c/FawSaqVZBV0/s1600/dsc01094.picasaweb.jpg:a "Nest Protect 2 — вид снизу.")

Изначально батареи отключены изолирующей вставкой для предотвращения их разряда. Батарейки, как уже упоминалось выше, литиевые, долгоиграющие, тип //Energizer Ultimate Lithium L91//, размер //AA//.
![](img:3.bp.blogspot.com/-GY4qiiTiHfU/VlrPOVc3o6I/AAAAAAAAk2c/eHewDU37Qp4/s1600/dsc01096.picasaweb.jpg:a "Nest Protect 2 — батарейный отсек.")

Никаких конкретных цифр о том, какой у прибора пробег от этих батарей, компания не приводит, ограничиваясь утверждением, что срок их службы «многолетний» (//«All Nest Protect models are designed for multiple years of battery life.»//). Судя по тому, что первые предупреждения //Protect// начинает выдавать за полгода до полного разряда, речь действительно идёт о годах.
...
![](img:1.bp.blogspot.com/-JUymlNnphFc/VlrPOU4wHWI/AAAAAAAAk2c/mmTZ9jT9cf8/s1600/dsc01097.picasaweb.jpg:a "Nest Protect 2 — сетевой кабель.")

Крепёжное кольцо с «лицевой» стороны (со стороны детектора) имеет резиновый уплотнитель по периметру:
![](img:3.bp.blogspot.com/-M_0VKHDHJ5w/VlrPOQ6mdcI/AAAAAAAAk2c/Cz1vSsk357E/s1600/dsc01098.picasaweb.jpg:a "Nest Protect 2 — крепёжное кольцо.")

То же самое с обратной стороны, для непонятливых нарисован зачёркнутый шуруп:
![](img:4.bp.blogspot.com/-70NhPGfrSlI/VlrPOdrzdBI/AAAAAAAAk2c/GjZGrKsdzJY/s1600/dsc01099.picasaweb.jpg:a "Nest Protect 2 — крепёжное кольцо, вид сзади.")

## Регистрация и настройка {#setup}

Согласно инструкции, перво-наперво необходимо установить приложение //Nest// на телефон или планшет (поддерживаются только //Android// и //iOS//). Кроме того, необходим Wi-Fi с доступом в Интернет и аккаунт //Nest//.

После этого запустить приложение, выбрать продукт //Nest Protect//, и оно предложит сканировать тот самый индивидуальный QR-код на дне прибора. Главный экран приложения приведён на скриншоте справа — если у вас есть другие продукты //Nest// (камера, термостат), они тоже появятся здесь.
![](img:3.bp.blogspot.com/-lrKhFu7LA7Q/Vlyy04Hc56I/AAAAAAAAk3Y/xhW7YmFKrzw/s1600/nest-app-02.picasaweb.png:a "Nest App. Добавление нового устройства и главный экран.")
...
Нажав на //Protect// на главном экране, попадаешь в раздел диагностики. Здесь можно запустить внеочередную проверку для каждого устройства, а также просмотреть историю предупреждений:
![](img:3.bp.blogspot.com/-QIs71DyAyX0/Vlyy0ysq1oI/AAAAAAAAk3Y/ktOU05prElg/s1600/nest-app-01.picasaweb.png:a "Nest App. Диагностика и история.")

Каждый из экземпляров //Nest Protect// настраивается индивидуально: у каждого можно независимо настроить функции ночника (//Pathlight//), «обещания на ночь» (//Nightly Promise//), детектора пара (//Steam Check//), предварительного предупреждения о том, что скоро сирена завоет по полной (//Heads-Up//), времени суток для ежемесячного теста, а также выбрать язык (в дополнение к двум, предлагавшимся при включении питания):
![](img:3.bp.blogspot.com/-3nNQMHmXIHs/Vlyy0633rjI/AAAAAAAAk3Y/8KnPCdf0hfE/s1600/nest-app-03.picasaweb.png:a "Nest App. Настройки детектора.")

В целом приложение очень простое и понятное.

Интересно, что видеть статус своих «протектов» можно через Интернет откуда угодно, но для запуска проверки сигнализации используется //Bluetooth//, поэтому необходимо находиться в непосредственной близости от них. И это замечательно, потому что никакой кулхацкер гарантированно не разбудит тебя воем сирены посреди ночи.
...
![](img:1.bp.blogspot.com/-QtFm6uEXm7s/VlrPOdycoqI/AAAAAAAAk2c/5Y8-Ext56C4/s1600/dsc01100.picasaweb.jpg:a "Nest Protect 2. Потолочное крепление наискось.")
...
Провод питания подключается к проводке прилагаемыми клеммами, после этого штекер вставляется в соответствующий разъём на дне, девайс вставляется в пазы крепёжного кольца и поворачивается так, чтобы выглядело *красиво*. Довольный //Protect// благодарно светится голубым светом:
![](img:2.bp.blogspot.com/-ZTziIeTzXvA/VlrPOXUtRMI/AAAAAAAAk2c/KYoxRkcGUN4/s1600/dsc01103.picasaweb.jpg:a "Nest Protect 2 на потолке.")
...
Никаких нареканий ни к работе, ни к конструкции детектора нет. Качество исполнения у //Nest Protect 2// превосходное и оставляет лишь положительные эмоции. Заметна исключительная продуманность каждого элемента, к тому же это уже вторая версия девайса. [Веб-сайт производителя](https://nest.com/smoke-co-alarm/meet-nest-protect/) тоже производит приятное впечатление — чувствуется забота о пользователе (любой наверняка может привести множество контрпримеров от других производителей).

Основное преимущество детектора //Nest// перед обычным — это то, что вместо глупой сирены он может внятно объяснить, *что* и *где именно* произошло на выбранном пользователем языке. Например: «Внимание, в кухне обнаружен дым» (//Heads-up, there's smoke in the kitchen!»//). То, что помещение, где он находится, называется «кухня», задаётся в настройках устройства.
...
Основного своего предназначения — обнаружения пожара — устройству выполнить пока, к счастью, не довелось. В целях тестирования я поджёг и задул спичку под носом у прибора, тот немедленно отреагировал предупреждением: «Дым в спальне, проверьте, что происходит. Может зазвучать сирена, сирена громкая». Если вы уверены, что всё в порядке, тревогу можно отменить в мобильном приложении (//Silence//).
![](img:4.bp.blogspot.com/-glh8lW3NFOM/Vl63AYVEQ4I/AAAAAAAAk4Y/mM9JnIRgyIA/s1600/nest-app-04.picasaweb.png:a "Nest Protect 2: реакция на дым и раздел «Сообщения».")

Также в аккаунте //Nest// хранится список событий, которые можно просмотреть в разделе //Messages//. Там фиксируются, например, отключения электропитания (скриншот справа).
...
На данный момент //Nest Protect 2// официально доступен лишь в следующих странах:
...
* ![](icon:minus) Непонятно, что будет с детектором, если компания //Nest Labs// перестанет существовать
